...
_SYMPHONY_PREFIX = '_SYMPH_'


class TmuxCluster():
  def __init__(self, name, start_dir, preamble_cmd=None, dry_run=False):
    """
    Args:
      name: Name of this TmuxCluster, corresponding to the Tmux server
    ...
    self._server_name = _SYMPHONY_PREFIX + name
    self._tmux = TmuxRunner(self._server_name, start_dir)
    self._tmux_sessions = dict()

  def _tmuxify(self, string):
    return _SYMPHONY_PREFIX + string

  def add_experiment(self, experiment_name):
    name = self._tmuxify(experiment_name)
    if name in self._tmux_sessions:
      raise errors.ResourceExistsError(
          "Experiment {} already exists".format(experiment_name))
    self._tmux_sessions[name] = self._tmux.add_session(name)

  def get_experiment(self, experiment_name):
    name = self._tmuxify(experiment_name)
    return self._tmux_sessions[name]


class __TmuxCluster(object):
    """
